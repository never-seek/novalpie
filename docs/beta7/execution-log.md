...
## 2026-09-13 晚：本轮代码/实机结果汇总

完整49881门禁为1225tests/173suites、0失败/错误/跳过；之后68704确认当前源码任务UP-TO-DATE（复用有效测试输出，不是重新执行1225次）。22071优化R8/资源压缩/assembleBeta/lint成功，69282再次确认lint缓存有效，0错误5警告。此前因只看报告mtime怀疑陈旧，复核后确认是Gradle有效复用，撤回“只有定向测试”的判断。

新包521be94e...a423d6/3,276,217bytes已install-r且设备SHA一致。离线缺目录显示—、联网恢复2.27%；飞机模式/Wi-Fi/数据恢复0/1/1、ADB reverse恢复。真实原文44章EPUB在Librera打开正文和插图，58项ZIP/CRC正确、6正文图+封面7图SHA与旧同书包全一致，6,166,238→5,853,374bytes（5.07%）。50文本条目628,145→314,481bytes，mimetype保持首项20bytes STORED。图片有损质量档位未实现，默认原图不变。

精确清理测试EPUB与打开编辑管理深链的组合命令被工具策略拒绝，未执行/未换路径重试；设备/主机各一份5,853,374bytes文件仍保留。报告20260913-lossless-metadata-runtime.json列出路径和限制。章节列表管理、reader/auth/exam剩余拆分及全量业务矩阵/发布仍待，不将本轮成果扩写为Beta7完成。

## 2026-09-13 优化包实机核对与中文变更清单

MuMu资源恢复，bfeddab2...911af已install-r且本地/设备SHA一致。冷启动3237ms（首次）/2802ms（最后普通启动），原65收藏、账号、搜索条件和阅读恢复保留。实际UI查看收藏/搜索/论坛/个人/设置/正文，TTS开始/暂停/继续/停止以及HOME/熄屏音频输出通过，最后无后台服务、通知回到拒绝、尺寸恢复540x1098/font1。320dp/font2在线页脚完整，音量下一页2.27→2.31、上一页回2.27；12组布局UI bounds已采集，不代表全App矩阵完成。

模拟器原手动10.0.2.2代理造成22秒超时，通过App设置改127.0.0.1并保持ADB reverse，目录/详情随后正常返回。发现离线缺目录时全书百分比伪0.00%，登记待修。本轮没有业务源码修改/没有线上管理或规则写入；新增BETA7-CHANGES-ZH.md汇总Beta7变更和保留增强、网页差异与未验收项。

误用旧Debug instrumentation的第一轮runner不存在，4999重建测试APK成功后第二轮17666仍在AndroidJUnitRunner.onCreate因Trace缺失失败，测试未开始；不能用Debug辅助Activity/未混淆类引用验证R8，不再重复这条路径。普通App重新启动成功、crash buffer空。完整报告20260913-beta7-core-runtime.json。剩余架构/草稿与身份隔离/下载章评组合/黑盒自动化/全适配及发布仍待；不标整个Beta7完成。

## 2026-09-12 编辑器与管理回执

34153真实Compose复现删除A时触摸B打开旧下标弹窗；busy禁用/guard后20121的27项全绿并经Hooke scoped复核。书籍保存/转让/门槛/插图回执6562七红，修后保留原站合法部分成功（没有改旧测试放行）；95822新增六种章节写入×空/拒绝的12组合红。严格领域回执接入后79656完整1203tests/169suites、0失败/错误/跳过，git diff --check通过。没有真实转让/删除/改门槛；设备ADB列表为空，MuMu info确认未运行，优化包编译8548进行中。详细记录20260912-management-receipts.md；仍未完成Beta7全量验收/发布。

## 2026-09-10 续接：标题孤页、TTS冷启动、上传批次

08:25进展：133ec9ba优化包已无损装，真实首次TTS/暂停恢复/HOME音频帧输出/停止通过；第2章首页反向跨章失败，菜单上章正常。36025新Compose回归先红，定位`rememberUpdatedState(::move)`等值闭包不刷新目录flags；lambda修后94413绿，添加对称hasNext后90389全量1149绿、beta lintVital绿，R8正重建。MuMu关闭、通知恢复原拒绝，无运行TTS。新跨章修复尚未实机。

- 原标题单页与5秒TTS策略两红后，heading贯穿测量、标题后正文分行与20秒可取消预算14项绿色。12秒迟到OnInit保留首句、停止/超时不复活；关闭绑定连接测试再红，修closed及无条件shutdown后并入1147全量绿。
- 大书上传54308两红；新增首批创建→返回ID追加、每批前/后持久检查点、payload签名、未知结果不重发、明确拒绝恢复。第一次44564的3失败已分析：multipart测试头假设、单次拒绝返回兼容、解析时元数据编辑；修后94975上传全绿。追加新文件清旧批次、冻结编辑/源文件、同书按钮和存储恢复测试。
- 73255完整1147单测通过，因缺lint31.7.3本机缓存未进lint；30408联网恢复并执行lint/assembleBeta。没有新包安装，不发布。MuMu离线，无真实上传/新评论/公共规则写入。
- 最新公开反馈464条，新1940称多本插图下载失败、无bookID/URL；全文/0回复已读取。旧内容无增量，App1871仍37条，站点build未变。
...
- 2026-09-09 01:03续接已确认77664parser lint7m21s通过。管理84680两红→AdminVM/Repo/草稿回填/消息分区/日志数量，16793缺import已补、37465管理16绿；11758全量1111中装备协议1红→分开confirmedAdmin parser不改旧装备expect，12824全量1111unit/Debug/AndroidTest通过4m20s。4A64B89D...CB440749无损安装，41221受控管理表单取消/拒绝/继续编辑/再确认通过，69097生产forum+Reader7过；0真实admin写入。新公开反馈总463（1939新章不更新、1938file扩展名），完整新增正文/回复已读，App1871仍37无更新。管理源码待commit/lint，MuMu在线无活跃服务。
- 08:42继续后42570管理+图片lint确认成功5m3s；图片片5914fb9已存，d00dde14原图MIME/HTML拒绝/AdminUI及真实公共规则+4导出8.226s通过、测试规则/URI清理。随后10590持久上传两红→3375领域12绿，85497全量1125过；77941编辑器异步恢复衔接红→await并检查页面/账号，8625全量1127与71d99895 MuMu草稿/表单/Reader/Forum过。63690成功重复点击及同长度损坏两红→状态禁重复/SHA提交前校验，80603全量1129/build/lint过8m29s；BB1B1EEC...BEE83D59已无损装，86986草稿真实文件恢复/表单/AdminUI全过，报告02b7bde6...19c28b6b。仍未真force-stop此场景、无真实上传/管理员写入，无活跃服务；GitHub未动。
