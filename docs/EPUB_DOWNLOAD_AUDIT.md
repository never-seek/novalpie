...
## 2026-08-24 目标作品 356636 反馈复核

针对论坛评论 `4396`/回复 `2305` 的“无封面、插图重复、图片数量约 2500”描述，本轮对
`356636《变成魔法少女了??》` 做了真实原生下载后的 ZIP/XHTML 审计。结果如下：

| 项目 | 结果 |
| --- | ---: |
| 章节 XHTML | 130 |
| 源 TXT 图片 occurrence | 3752 |
| EPUB 正文图片条目 | 3752 |
| EPUB 正文图片引用 | 3752 |
| 独立封面页/封面图片 | 有 / 有 |
| 缺失图片引用 | 0 |
...
源站章节目录的 `image_count` 合计为 2474，但源 TXT 实际含有 3752 个图片占位符；原生
writer 的 3752 条输出与正文 occurrence 一一对应。源 TXT 本身含 552 组相邻相同内容，
且有 1235 组内容被多个不同条目复用。这里保留不同 occurrence 是为了与网站生成器的
章节引用契约一致，不能按 URL/hash 合并，否则会把合法的章节图片引用删掉。

该产物还包含可解析的 `cover.xhtml`、OPF cover manifest/spine 项；封面源图和 EPUB 内封面
字节一致。第三方阅读器（尤其番茄、起点、多看）尚未在 MuMu 安装验证，因此本审计只证明
EPUB 文件和原生下载链路完整，不把第三方应用兼容性写成已验证事实。完整 JSON：
