...
> 2026-09-05 时效说明：本文件“当前 APK”指各条所附旧哈希，不是 Beta 7。Beta 7 使用
> [新的需求/冲突清单](beta7/requirements.md) 和 [当前验证门禁](beta7/verification.md)。
> 后续用户已禁止正文复制并移除轮盘，下面保留的早期复制验收不能重新作为需求。
...
### 2026-09-01：公共全文替换、原生导出与投票帖反馈当前 APK 验收

- **B08 / 公共规则策略：实机通过。** 新增设备默认开关（安全默认关闭）、按书覆盖、单条公共规则隐藏和“恢复跟随默认设置”。书籍 `358153` 使用真实共享规则 `令爱 → 大小姐`：开启“本书应用公共规则”后 Reader 正文显示 `大小姐`；只关闭这一条时在主开关仍开的前提下恢复 `令爱`；恢复跟随默认设置后回到默认关闭。证明“浏览全部规则”不再隐式改变阅读文本。证据：`agent-bridge/screenshots/20260901-reader-global-rule-applied.png`、`20260901-reader-individual-global-rule-hidden.png`、`20260901-reader-global-rule-disabled-restored.png` 与相应 UI tree。
- **C01 / C02 / 导出替换：实机通过。** 原生下载弹窗明确提供“原文下载”和“应用当前替换规则”。对 `358153` 选择后，TXT 为 1,200,515 bytes；EPUB 为 1,633,519,702 bytes，`unzip -t` 成功，85 XHTML、548 正文图引用/资源、独立封面 1。两种导出正文均为 `令爱=0`、`大小姐=72`，确认冻结规则快照进入 TXT/EPUB；图片 URL 和资源没有被文本替换改写。仅删除本轮精确命名的 TXT/EPUB，未触碰既有下载。证据：`screenshots/20260901-{txt-download-choice,epub-download-choice,epub-download-complete}.png`、`artifacts/20260901-qa-replaced-358153-*`。
- **投票帖阅读器反馈：实机/单测闭环。** 同章阅读位置新增持久化 anchor；`am force-stop` 后重开仍在同一段而不是章节首屏，证据 `screenshots/20260901-reader-viewport-restored-after-force-stop.png`。HTML/Markdown 斜体、下划线、删除线的重叠 span 现在不会重复正文；内容宽度控制移至“布局”；详情页简介不再放置三个低频管理按钮。`/api/reader/progress` 当前登录会话实测完成；繁体模式正文显示 `马车 → 馬車`，测试后恢复原文模式。章评默认折叠已有现行实现与 `ReaderPresentationTest` 覆盖；横屏双页按用户既有决定不纳入。
- **最终门禁：** `:app:testDebugUnitTest :app:testReleaseUnitTest :app:lintDebug :app:assembleDebug --offline --no-daemon --max-workers=1 -Pkotlin.incremental=false --console=plain` 成功；Debug/Release 各 92 suites / 818 tests / 0 failures / 0 errors / 0 skipped，Lint `No issues found`，`git diff --check` 通过。APK `D1D104BAC49362A666CA810FBC6970A12C019B2AB999382D3AD922DC2B424696` 已以 `adb install -r` 安装到 MuMu，设备 base APK hash 一致。
